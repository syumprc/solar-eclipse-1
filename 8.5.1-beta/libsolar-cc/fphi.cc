...
static Eigen::ArrayXXd find_max_loglik(const size_t precision, Eigen::VectorXd Y, Eigen::MatrixXd aux, Eigen::MatrixXd X, double & result_loglik, double & result_variance){
    ...
    if(qr.isInvertible()){
        Eigen::ArrayXd errors = qr.inverse().diagonal().cwiseSqrt();
        for(size_t i = 0 ; i < beta.rows();i++){
...
static int load_fphi_matrices(Tcl_Interp * interp, Eigen::VectorXd  & trait_vector, Eigen::MatrixXd  & covariate_matrix, Eigen::MatrixXd & auxiliary_matrix, Eigen::MatrixXd & eigenvectors,\
    ...
    try{
        ...
        string error_string = "Parse Expression Error with term: " + string(error.term_name);
        RESULT_LIT(error_string.c_str());
        return TCL_ERROR;
    }catch(Solar_File_Error & error){
        string error_string = "Solar File Error filename: " + string(error.filename);
        RESULT_LIT(error_string.c_str());
        return TCL_ERROR;
    }catch(Expression_Eval_Error & error){
        string error_string = "Expression Evaluation Error id: " + string(error.id) + " term: " + string(error.term) + " row: " + to_string(error.row) + " col: " + to_string(error.col);
        RESULT_LIT(error_string.c_str());
        return TCL_ERROR;
    }catch(Misc_Error & error){
        string error_string = "Miscellaneous Error message: " + string(error.msg);
        RESULT_LIT(error_string.c_str());
        return TCL_ERROR;
    }
...
extern "C" int runfphiCmd(ClientData clientData, Tcl_Interp * interp,
    ...
    success = load_fphi_matrices(interp, trait_vector, cov_matrix, aux, eigenvectors,trait_list[0]);
    ...
    if(converge){
        ...
        Eigen::ArrayXXd parameters =find_max_loglik(9, Y, aux, X, loglik,  variance);
