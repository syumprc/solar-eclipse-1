...
solar_mle_setup::solar_mle_setup(vector<string> input_terms, const char * filename,Tcl_Interp * input_interp,  bool ped_switch){
    if(input_terms.size() == 0){
       throw Misc_Error(Strdup("blank list of expressions"));
    }
    interp = input_interp;
    ...
    for(int index = 0; index < input_terms.size(); index++){
        ...
        if(result == TCL_ERROR){
            throw Parse_Expression_Error(Strdup (input_terms[index].c_str()));
        }
    }
    ...
    if(result == TCL_ERROR){
        throw Solar_File_Error(Strdup(filename));
    }
    n_subjects = ids.size();
    ...
    for(int row_idx = 0; row_idx < n_subjects; row_idx++){
        for(int col_idx = 0; col_idx < n_expressions; col_idx++){
            try{
                ...
                CURRENT_LOADER = 0;
                throw Expression_Eval_Error(Strdup(ids[row_idx].c_str()), Strdup(expression_names[col_idx].c_str()), row_idx, col_idx);
           }
        }
        id_index++;
    }
...
void solar_mle_setup::compute_eigen_data(){
    ...
    if (!pm2) {
        ...
        pm2 = Matrix::find("phi2");
        if(!pm2){
            const char * msg = "Phi2 matrix could not be loaded";
            throw Misc_Error(Strdup(msg));
        }
    }
